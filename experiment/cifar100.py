...
from utils.model.models import create_resnet18
...
def create_task(batch_size:int=20, crop_height:int = DEFAULT_CROP_HEIGHT, crop_width:int = DEFAULT_CROP_WIDTH):
    crop_shape = (crop_height, crop_width, 3)
    
    # Training dataset distorts image
    train_preprocessing_fn = create_preprocess_fn(batch_size, crop_shape=crop_shape, distort=True)
    test_preprocessing_fn = create_preprocess_fn(batch_size, crop_shape=crop_shape, distort=False)
