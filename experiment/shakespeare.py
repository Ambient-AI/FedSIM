...
from utils.model.models import create_lstm_shakespeare
...
def create_task(batch_size:int = 4, sequence_length:int = DEFAULT_SEQUENCE_LENGTH):
    """Creates a preprocessed shakespeare NCP task"""
    logging.basicConfig(format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S', level=logging.INFO)
    preprocessing_fn = create_preprocess_fn(batch_size)
    ...
    def model_fn():
        model = create_lstm_shakespeare(VOCAB_LENGTH, sequence_length)
        loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        #metrics=[MaskedCategoricalAccuracy(masked_tokens=[pad_token])]
