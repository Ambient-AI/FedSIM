...
def create_task(num_epochs:int, batch_size:int = 16, 
                sequence_length:int = DEFAULT_SEQUENCE_LENGTH,
                vocab_size:int = DEFAULT_WORD_VOCAB_SIZE,
                num_out_of_vocab_buckets:int = 1):
    """Creates a preprocessed stackoverflow NWP task"""
    logging.info("Downloading vocab dictionary from Google Storage API")
    ...
    preprocessing_fn = create_preprocess_fn(num_epochs, batch_size, vocab, sequence_length, num_out_of_vocab_buckets=num_out_of_vocab_buckets)
    ...
    def metrics_builder():
        return [keras_metrics.MaskedCategoricalAccuracy(name='accuracy', masked_tokens=[pad_token]),
                keras_metrics.MaskedCategoricalAccuracy(name='acc w/o oov', masked_tokens=[pad_token]+oov_tokens),
                keras_metrics.MaskedCategoricalAccuracy(name='acc w/o oov or eos', masked_tokens=[pad_token, eos_token] + oov_tokens)]
    ...
    def model_fn():
        model = models.create_lstm_stackoverflow(extended_vocab_size)
